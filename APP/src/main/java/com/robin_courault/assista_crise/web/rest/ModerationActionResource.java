...
/**
 * REST controller for managing {@link com.robin_courault.assista_crise.domain.ModerationAction}.
 */
@RestController
...
public class ModerationActionResource {
    ...
    public ResponseEntity<ModerationActionDTO> createModerationAction(@Valid @RequestBody ModerationActionDTO moderationActionDTO)
        ...
        return ResponseEntity.created(new URI("/api/moderation-actions/" + moderationActionDTO.getId()))
    ...
    /**
     * {@code GET  /moderation-actions} : get all the Moderation Actions.
     *
     * @param pageable the pagination information.
     * @param eagerload flag to eager load entities from relationships (This is applicable for many-to-many).
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of Moderation Actions in body.
     */
    @GetMapping("")
    public ResponseEntity<List<ModerationActionDTO>> getAllModerationActions(
        ...
        Page<ModerationActionDTO> page;
        if (eagerload) {
            page = moderationActionService.findAllWithEagerRelationships(pageable);
        } else {
            page = moderationActionService.findAll(pageable);
        }
