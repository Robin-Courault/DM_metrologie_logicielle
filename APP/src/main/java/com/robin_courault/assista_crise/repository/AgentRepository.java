...
public interface AgentRepository extends JpaRepository<Agent, Long> {
    ...
    @Query(
        value = "select agent from Agent agent left join fetch agent.utilisateur left join fetch agent.autorite",
        countQuery = "select count(agent) from Agent agent"
    )
    Page<Agent> findAllWithToOneRelationships(Pageable pageable);

    @Query("select agent from Agent agent left join fetch agent.utilisateur left join fetch agent.autorite")
    List<Agent> findAllWithToOneRelationships();

    @Query("select agent from Agent agent left join fetch agent.utilisateur left join fetch agent.autorite where agent.id =:id")
